...
def test_get_preparar_renderiza_formulario_com_meios(conn: sqlite3.Connection, client: FlaskClient) -> None:
    contato = perfis.inserir_ou_atualizar_contato(conn, "Ana", None, "ana@x.com", None, None, None, AGORA)
    assert contato.id is not None
    conn.commit()

    resposta = client.get(f"/contatos/{contato.id}/preparar")

    assert resposta.status_code == 200
    texto = resposta.get_data(as_text=True)
    assert "Preparar consulta" in texto
    assert 'name="assunto"' in texto
    assert 'name="meio"' in texto
    assert "pessoalmente" in texto
    assert "whatsapp" in texto


def test_get_preparar_devolve_404_para_contato_inexistente(client: FlaskClient) -> None:
    resposta = client.get("/contatos/9999/preparar")

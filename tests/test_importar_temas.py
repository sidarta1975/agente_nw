...
def test_importar_temas_grava_perfil_e_temas(tmp_path: Path) -> None:
    ...
    resultado = importar_temas(str(RAIZ / "temas.exemplo.yaml"), simular=False, conexao_bd=conn)
...
def test_importar_temas_de_novo_nao_duplica(tmp_path: Path) -> None:
    ...
    importar_temas(str(RAIZ / "temas.exemplo.yaml"), simular=False, conexao_bd=conn)
    importar_temas(str(RAIZ / "temas.exemplo.yaml"), simular=False, conexao_bd=conn)
